...
//   3 = Photo (handled by /api/upload/photo, nothing to save here)
...
export async function PUT(req: NextRequest) {
  ...
  try {
    if (step === 1) {
      ...
      // Photo step — photo upload handled by /api/upload/photo
      // This step just advances the wizard, nothing to persist
      // (Photo URL is already saved to profiles.photo_url by the upload endpoint)
