...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    entries: result.rows.map((f: Record<string, unknown>) => ({
      id: f.id, userId: f.user_id, email: f.email, message: f.message,
      pageUrl: f.page_url, reportedProfileId: f.reported_profile_id,
      reportedSlug: f.reported_slug, feedbackType: f.feedback_type,
      status: f.status, adminNotes: f.admin_notes || '',
...
export async function PUT(req: NextRequest) {
  ...
  if (adminNotes !== undefined) { sets.push(`admin_notes = $${idx++}`); params.push(adminNotes); }
