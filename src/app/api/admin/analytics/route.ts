...
async function getUmamiToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { lastAuthError = `HTTP ${res.status}: ${(await res.text().catch(() => '')).slice(0, 200)}`; return null; }
    const data = await res.json();
    cachedToken = { token: data.token, expiresAt: Date.now() + 10 * 60 * 1000 };
    return data.token;
  } catch (err) { lastAuthError = err instanceof Error ? err.message : String(err); return null; }
}

async function umamiGet(path: string, token: string) {
  const res = await fetch(`${UMAMI_API_URL}${path}`, { headers: { Authorization: `Bearer ${token}` } });
...
export async function GET(req: NextRequest) {
  ...
  if (!session?.user?.email || !isAdmin(session.user.email)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  if (!WEBSITE_ID) return NextResponse.json({ error: 'UMAMI_WEBSITE_ID env var is empty', configured: false }, { status: 200 });

  const token = await getUmamiToken();
  if (!token) return NextResponse.json({ error: `Umami auth failed (${UMAMI_API_URL}, user=${UMAMI_USERNAME}): ${lastAuthError}`, configured: false }, { status: 200 });

  const { searchParams } = new URL(req.url);
  const period = searchParams.get('period') || '7d';
  const now = Date.now();
  let startAt: number, unit: string;
  switch (period) {
    ...
    default: startAt = now - 7*24*60*60*1000; unit = 'day';
  }
