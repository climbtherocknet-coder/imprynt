...
export async function GET() {
  ...
  const result = await query(
    `SELECT id, email, source, invited, invited_at, created_at FROM waitlist ORDER BY created_at DESC`
  );

  return NextResponse.json({
    entries: result.rows.map((w: Record<string, unknown>) => ({
      id: w.id, email: w.email, source: w.source, invited: w.invited,
      invitedAt: w.invited_at, createdAt: w.created_at,
    })),
  });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized)) return NextResponse.json({ error: 'Invalid email' }, { status: 400 });

  const existing = await query('SELECT id FROM waitlist WHERE email = $1', [normalized]);
  if (existing.rows.length > 0) return NextResponse.json({ error: 'Already on waitlist' }, { status: 409 });

  await query("INSERT INTO waitlist (email, source) VALUES ($1, 'admin')", [normalized]);
...
export async function PUT(req: NextRequest) {
  ...
  const codeResult = await query(
    `INSERT INTO invite_codes (code, created_by, max_uses, note, granted_plan) VALUES ($1, $2, 1, $3, $4) RETURNING id`,
    [inviteCode, session.user.email, 'Waitlist invite', plan]
  );

  const inviteCodeId = codeResult.rows[0].id;
  const result = await query(
    `UPDATE waitlist SET invited = true, invited_at = NOW(), invite_code_id = $1 WHERE id = $2 AND invited = false RETURNING id, email`,
    [inviteCodeId, id]
  );
  ...
  sendWaitlistInviteEmail(email, inviteCode).catch((err) => console.error('Waitlist invite email failed:', err));
