...
export async function GET() {
  ...
  return NextResponse.json({
    codes: result.rows.map((c: Record<string, unknown>) => ({
      id: c.id, code: c.code, createdBy: c.created_by, maxUses: c.max_uses,
      useCount: c.use_count, expiresAt: c.expires_at, note: c.note || '',
      grantedPlan: c.granted_plan || 'free', createdAt: c.created_at,
    })),
  });
}
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    await query(
      `INSERT INTO invite_codes (code, created_by, max_uses, expires_at, note, granted_plan) VALUES ($1, $2, $3, $4, $5, $6)`,
...
export async function PUT(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'Missing invite code ID' }, { status: 400 });
  ...
  if (code !== undefined) {
    ...
    if (!clean) return NextResponse.json({ error: 'Code cannot be empty' }, { status: 400 });
    sets.push(`code = $${idx++}`); params.push(clean);
  }
  if (maxUses !== undefined) { sets.push(`max_uses = $${idx++}`); params.push(maxUses === 0 ? null : maxUses); }
  if (expiresInDays !== undefined) {
    sets.push(`expires_at = $${idx++}`);
    params.push(expiresInDays ? new Date(Date.now() + expiresInDays * 24 * 60 * 60 * 1000).toISOString() : null);
  }
  if (note !== undefined) { sets.push(`note = $${idx++}`); params.push(note?.trim().slice(0, 255) || null); }
  if (grantedPlan !== undefined) {
    const vp = ['free', 'premium_monthly', 'premium_annual', 'advisory'];
    sets.push(`granted_plan = $${idx++}`); params.push(vp.includes(grantedPlan) ? grantedPlan : 'free');
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (check.rows.length === 0) return NextResponse.json({ error: 'Code not found' }, { status: 404 });
  if (check.rows[0].use_count > 0) return NextResponse.json({ error: 'Cannot delete a code that has been used. Edit it instead.' }, { status: 400 });
